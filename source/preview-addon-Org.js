// preview-addon.js — docked preview pane with TIFF support (canvas), never covers the form
(function () {
  const PANE_W = 420;
  const CSS = `
  ...
  pane.innerHTML = `
    ...
    <div class="dbm-preview-body">
      ...
      <canvas id="dbm_prev_canvas" class="dbm-preview-media" hidden></canvas>
  ...
  const canvEl  = pane.querySelector('#dbm_prev_canvas');
  ...
  const isImageNoTiff = p => /\.(png|jpe?g|webp)$/i.test(p);
  const isTiff    = p => /\.(tif|tiff)$/i.test(p);
  const looksLikePath = s => /^[A-Za-z]:\\/.test(s);

  function hideAll(){ imgEl.hidden = true; videoEl.hidden = true; canvEl.hidden = true; emptyEl.hidden = false; }

  async function setPreview(fp){
    if(!fp){ nameEl.textContent='No file selected'; hideAll(); return; }
    nameEl.textContent = fp; emptyEl.hidden = true;

    if (isImageNoTiff(fp)) {
      imgEl.src = toFileURL(fp);
      imgEl.hidden = false; videoEl.hidden = true; canvEl.hidden = true;
      return;
    }
    if (isVideo(fp)) {
      videoEl.src = toFileURL(fp);
      videoEl.hidden = false; imgEl.hidden = true; canvEl.hidden = true;
      return;
    }
    if (isTiff(fp)) {
      try {
        // require UTIF in renderer
        const ok = window.UTIF && typeof window.UTIF.decode === 'function';
        if (!ok) { throw new Error('UTIF decoder not found'); }
        const u8 = await window.dbm.readBinary(fp);   // Uint8Array
        const decoded = window.UTIF.decode(u8);       // {width,height,rgba}
        const { width, height, rgba } = decoded;
        const ctx = canvEl.getContext('2d');
        canvEl.width = width; canvEl.height = height;
        const imgData = new ImageData(new Uint8ClampedArray(rgba), width, height);
        ctx.putImageData(imgData, 0, 0);
        canvEl.hidden = false; imgEl.hidden = true; videoEl.hidden = true;
        return;
      } catch (e) {
        console.error('TIFF preview failed:', e);
